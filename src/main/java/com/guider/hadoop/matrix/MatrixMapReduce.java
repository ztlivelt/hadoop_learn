            ...
            if (name.contains("matrixa.txt")) {
                for (int i = 1; i <= colNum; i++) {
                    ...
                    for (int j = 1; j <= ss.length; j++) {
                        Text aValue = new Text("a," + j + "," + ss[j - 1]);
                        context.write(k, aValue);
                    }
                }
                rowIndexA++;
                System.out.println("A:    " + rowIndexA);
            } else if (name.contains("matrixb.txt")) {
                for (int i = 1; i <= rowNum; i++) {
                    for (int j = 1; j <= ss.length ; j++) {
                        Text k = new Text(i + "," + j);
                        Text aValue = new Text("b," + rowIndexB + "," + ss[j - 1]);
                        context.write(k, aValue);
                    }
                }
                rowIndexB++;
                System.out.println("B:    " + rowIndexB);
            }
        }
    }

    public static class MatrixReduce extends Reducer<Text, Text, Text, IntWritable> {
        @Override
...
//        job.setGroupingComparatorClass(cls);
    ...
    public static void main(String[] args) throws Exception {
        ...
        //参数
        args = new String[]{
                "hdfs://bigguider22.com:8020/user/root/matrix/input",
                "hdfs://bigguider22.com:8020/user/root/matrix/output"
        };
        //跑我们的任务
        handleOutputFile(args[1]);
        int status = ToolRunner.run(conf, new MatrixMapReduce(), args);
        System.exit(status);
    }
